...
`domain == 4`. Read TWO files for two different things: `learning-progress.json` for this domain's
**teaching status**, and **`stats.json` for this domain's real exam misses** (ids in `answered` whose latest
attempt is `last_correct:false`, kept to `domain == 4`). `stats.json` is the ONLY source of misses — never
