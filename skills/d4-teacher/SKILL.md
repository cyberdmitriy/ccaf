...
2. Read `$HOME/.claude/ccaf-progress/{fails-tracker.md,trap-log.md,profile.md,settings.json}`. Prioritise the user's OWN recorded D4 weak spots; if `profile.md` flags D4 weak, build broad coverage.
...
`domain == 4`: read `learning-progress.json` for this domain's status and gather this domain's misses. If
